            ...
            <input onclick="$('[name=action]').val($('#action').val()); $('#action_faq_form').submit()" value="Apply" class="btn" type="button">
       		<?php if(count($fcate_info[0])>0){?>
                	<select name="category" class="" style="width: 150px;" >
						...
						<option value="<?php echo $leaf->id;?>" <?php if($leaf->id == $cate_id){echo "selected";}?>>
							<?php
								echo $leaf->name;
							?>
						</option>
    	...
    	<form action="<?php echo admin_url();?>/faqs/do-action" method="post" id="action_faq_form">
	        ...
	        <div class="linecate2">
	            ...
	            <div id="row_<?php echo $v->id;?>">
		            ...
		            <?php if($v->active == 'yes'){?>
		            	<a href="javascript:void(0);" onclick="faqs_status('<?php echo $v->id;?>', 'no', '<?php echo $current_url;?>')"><img src="<?php echo base_url();?>public/admin/img/active.png" title="Active" alt="Yes" class="icon png" /></a>
					<?php }else{?>		            	
		            	<a href="javascript:void(0);" onclick="faqs_status('<?php echo $v->id;?>', 'yes', '<?php echo $current_url;?>')"><img src="<?php echo base_url();?>public/admin/img/pending.png" title="Suspend" alt="No" class="icon png" /></a>
		            <?php }?>
		            </div>
	            </div>
	        </div>
