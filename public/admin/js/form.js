 ...
 */
	...
	//Function delete(id), have been checked and working well
	function user_delete(user_id){
	    if(!confirm('Are you sure want delete this item?')) {
	        return false;
	    }
	    
		$.post(admin_url+'delete-adminstrator/', 'id='+user_id, function(msg) {
			if(msg == "yes") {
	...
	function user_status(user_id, status){
		$.post(
				admin_url+'change-status/',
				{acc_id: user_id, acc_status: status},
				function(msg){
					if(msg == "yes") {
						load_content('row_'+user_id, admin_url+'load-row/'+user_id);
