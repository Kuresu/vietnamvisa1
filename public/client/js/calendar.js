var numberOfDaysPerMonthHashTable = {
    '9,2007': 30,
    '10,2007': 31,
    '11,2007': 30,
    '12,2007': 31,
...
function ShiftDate() {
    ...
    if (ShiftY == 2008) {
        if (ShiftM == 12) {
            if (ShiftD == 11) {
                DateShift = DateShift + 3;
            }
            else if (ShiftD == 12) {
                DateShift = DateShift + 2;
            }
            else if (ShiftD == 13) {
...
function hideCalendar() {
    if (cal_crossObj) {
        ...
        if (document.getElementById("spanRight")) {
...
function getMonthTable(selYear, selMonth) {
    ...
    for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++) {
        ...
        dateMessage = "";
        if (bDayEnabled) sHTML += "<a class='" + sStyle + "' " + dateMessage + " title=\"" + sHint + "\" href='javascript:cal_seletion(" + datePointer + "," + selMonth + "," + selYear + ");'>&nbsp;" + datePointer + "&nbsp;</a>"
        else sHTML += "<label class='" + sStyle + "' " + sDisabled + " title=\"" + sHint + "\" >&nbsp;" + datePointer + "&nbsp;</label>"
        sHTML += "";
...
function constructCalendar() {
    ...
    if (bPrevEnabled) bPrevEnabled = chkEnableDay(1, cal_monthSelected, cal_yearSelected);
    if (typeof(chkEnableDay) == "function") {
...
function popUpCalendar(ctl,	element_id, OtherFunctions) {
    hideCalendar();
	...
	if (cal_bPageLoaded) {
		if (cal_crossObj.visibility == "hidden") {
			cal_ctlToPlaceValue	 = element_id;
			cal_dateFormat       = 'dd/mm/yyyy';

            var id = (element_id == 'check_in') ? '1' : '2';
			cal_dateSelected     = $('#day_'+id).val();
			cal_monthSelected    = $('#month_'+id).val() - 1;
			cal_yearSelected     = $('#year_'+id).val();

			if (isNaN(cal_dateSelected)||isNaN(cal_monthSelected)||isNaN(cal_yearSelected)) {
		...
		init_calendar();
		popUpCalendar(ctl, element_id, OtherFunctions);
	}
}
...
document.onkeypress = function hidecal1(e) {
    var IntKeyCode = 0;
    if (document.all) IntKeyCode = event.keyCode;
    else IntKeyCode = e.which;
    if (IntKeyCode == 27) {
        hideCalendar();
    }
}
...
function chkEnableDay(chkday, chkmonth, chkyear) {
    ...
    if (chkyear < yearNow || (chkyear - yearNow > 1)) retval = false;
    return retval;
}

function fnSetDate(element_id, nDay, nMonth, nYear) {
    nMonth++;
    $('#arr_day').val(nDay);
    $('#arr_mon').val(nMonth);
    $('#arr_year').val(nYear);    
...
function fnSetDate2(element_id, nDay, nMonth, nYear) {
    nMonth++;
    $('#dep_day').val(nDay);
    $('#dep_mon').val(nMonth);
    $('#dep_year').val(nYear);    
...
function fnSetDate3(element_id, nDay, nMonth, nYear) {
    nMonth++;
    $('#xxx_date').val(nMonth+'/'+nDay+'/'+nYear);    
}