	...
	<form action="<?php echo admin_url();?>/testimonials/search-results" method="post">
	    <div class="toppage">
	    	<span class="left">
	            ...
	            <input onclick="$('[name=action]').val($('#action').val()); $('#action_testimonials_form').submit()" value="Apply" class="btn" type="button">
	...
	<form action="<?php echo admin_url();?>/testimonials/" method="post" id="list-testimonials-form">
	    <div class="topart">
	    	<span class="left">
	    		<?php 
	    			$total_amount	=	count($this->testimonials_model->get_all());
	    			$active			=	$this->testimonials_model->count_active();
	    		?>
	    		<font class="number">(<?php if ($total_amount >0) {echo $total_amount;}else {echo "0";}?>)</font> <a href="javascript:;">All</a><font class="line">|</font><font class="number">(<?php if (count($active)>0) {echo $active;}else {echo "0";}?>)</font> <a href="javascript:;"><font color="#0b9901">Active</font></a><font class="line">|</font><font class="number">(<?php echo $total_amount - $active;?>)</font> <a href="javascript:;"><font color="#c60001">Pending</font></a><font class="line">
	        </span>
	        <span class="right1">
	        	<select style="width:50px;" name="perpage" id="perpage" onchange="changeTestimonialsNumber()">
    ...
    <div class="tableout">
    	<form action="<?php echo admin_url();?>/testimonials/do-action" method="post" id="action_testimonials_form">
			<div class="title1">
	            ...
	            <div class="column" style="width: 30%;">Content</div>
	            <div class="column" style="width: 13%;">Country</div>
	            <div class="column" style="width: 10%;">Rating</div>
	            <div class="column" style="width: 16%;">Date</div>
	            <div class="column" style="width: 5%;">Status</div>
	        </div>
	        <?php if(count($testimonials_list[0])>0){?>
	        <?php foreach ($testimonials_list as $k => $v){?>
	        <div class="linecate2">
	            ...
	            <div id="row_<?php echo $v->id;?>">
		            <div class="column" style="width:20%;" onmouseover="Hovercat('<?php echo $v->id;?>')" onmouseout="Outcat('<?php echo $v->id;?>')">
		                ...
		                <div class="action" id="neocat-<?php echo $v->id;?>">
		                    <img src="<?php echo base_url();?>public/admin/img/edit.gif" /><a href="javascript:void(0)" onclick="open_form('<?php echo admin_url(); ?>/edit-testimonials/<?php echo $v->id;?>')">Edit</a>|<img src="<?php echo base_url();?>public/admin/img/delete.gif" /><a href="<?php echo admin_url();?>/delete-testimonials/<?php echo $v->id;?>" onclick="return confirm('Do you want delete this testimonials?');"><font color="#be0000">Delete</font></a>
		                </div>
		            </div>
		            ...
		            <div class="column " style="width:13%; color: grey;"><?php echo $v->country;?></div>
		            <div class="column " style="width:10%; padding: 5px 0 0 0;">
		            	<?php for($i =1; $i <= 5; $i ++){ if($v->rating == $i){?>
		            		<img alt="" src="<?php echo base_url();?>public/admin/img/<?php echo $i.'_star_small.gif';?>">
		            	<?php }}?>
		            </div>
		            <div class="column " style="width:16%; color: grey;"><?php echo $v->time;?></div>
		            <div class="column" style="width:5%;">
		            <?php if($v->active == 'yes'){?>
		            	<a href="javascript:void(0);" onclick="testimonials_status('<?php echo $v->id;?>', 'no')"><img src="<?php echo base_url();?>public/admin/img/active.png" title="Active" alt="Yes" class="icon png" /></a>
					<?php }else{?>		            	
		            	<a href="javascript:void(0);" onclick="testimonials_status('<?php echo $v->id;?>', 'yes')"><img src="<?php echo base_url();?>public/admin/img/pending.png" title="Suspend" alt="No" class="icon png" /></a>
		            <?php }?>
		            </div>
	            </div>
	        </div>
...
<script type="text/javascript">
	function changeTestimonialsNumber(){
		$("#list-testimonials-form").submit();
	}

	var inform	=	'<?php echo $inform;?>';
	
	if(inform == 'delete testimonials success') {alert("Delete Successfully !");}

	$('#action_testimonials_form').iframer({
